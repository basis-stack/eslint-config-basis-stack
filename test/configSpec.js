...
the('config', () => {
  ...
  should('allow object & array propTypes in React components', () => {

    expect(config.rules['react/forbid-prop-types']).to.deep.equal([2, { 'forbid': ['any'] }]);
  });

  should('allow JSX closing bracket after last property', () => {

    expect(config.rules['react/jsx-closing-bracket-location']).to.deep.equal([2, 'after-props']);
  });
  ...
  should('allow JSX properties on same line as opening tag', () => {

    expect(config.rules['react/jsx-first-prop-new-line']).to.deep.equal([2, 'never']);
  });
  ...
  should('allow function parameters to be aligned (with first parameter)', () => {

    expect(config.rules['function-paren-newline']).to.deep.equal([2, 'never']);
  });
  ...
  should('allow JSX attributes to be aligned (with first attribute)', () => {

    expect(config.rules['indent'][2].ignoredNodes).to.deep.equal(['JSXAttribute', 'JSXSpreadAttribute']);
  });
  ...
  should('allow caps case names for decorators', () => {
    ...
    expect(config.rules['new-cap'][1].capIsNewExceptions).to.deep.equal(['Authenticate', 'Controller', 'Middleware', 'Route', 'Router', 'Get', 'Head', 'Post', 'Put', 'Delete', 'Options']);
  });
  ...
  should('allow mix-use newlines (inline & multi-line) for object curly braces', () => {

    expect(config.rules['object-curly-newline']).to.deep.equal(disabledResult);
  });
